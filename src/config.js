require('dotenv').config();

const DEFAULT_MODEL = 'gpt-3.5-turbo'; // 默认模型，仅在环境变量未设置时使用

const OPENAI_MODELS = process.env.OPENAI_MODELS 
  ? process.env.OPENAI_MODELS.split(',').map(model => model.trim())
  : [DEFAULT_MODEL];

module.exports = {
  OPENAI_API_KEY: process.env.OPENAI_API_KEY,
  OPENAI_BASE_URL: process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1',
  OPENAI_MODELS: OPENAI_MODELS,
  DEFAULT_MODEL: OPENAI_MODELS[0], // 使用环境变量中的第一个模型作为默认模型
  TELEGRAM_BOT_TOKEN: process.env.TELEGRAM_BOT_TOKEN,
  WHITELISTED_USERS: process.env.WHITELISTED_USERS ? process.env.WHITELISTED_USERS.split(',').map(id => parseInt(id.trim())) : [],
  UPSTASH_REDIS_REST_URL: process.env.UPSTASH_REDIS_REST_URL,
  UPSTASH_REDIS_REST_TOKEN: process.env.UPSTASH_REDIS_REST_TOKEN,
};
